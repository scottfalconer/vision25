...
// Production hosts should be provided explicitly through the environment.
$trusted_hosts = getenv('DRUPAL_TRUSTED_HOST_PATTERNS');
if ($trusted_hosts !== FALSE && trim($trusted_hosts) !== '') {
...
// DDEV's scaffolded wildcard host pattern is too broad for routine use.
if (getenv('IS_DDEV_PROJECT') == 'true' && ($ddev_site = getenv('DDEV_SITENAME'))) {
  $settings['trusted_host_patterns'] = [
    '^' . preg_quote($ddev_site, '/') . '\.ddev\.site$',
    '^localhost$',
    '^127\.0\.0\.1$',
  ];
}
...
// Load Acquia Cloud environment settings when available.
$ah_site_group = getenv('AH_SITE_GROUP');
$ah_site_environment = getenv('AH_SITE_ENVIRONMENT');
if ($ah_site_group && $ah_site_environment) {
  $ah_site_name = $ah_site_group . $ah_site_environment;
  $acquia_settings_candidates = [
    "/var/www/site-php/{$ah_site_name}/D11-{$ah_site_environment}-{$ah_site_group}-settings.inc",
    "/var/www/site-php/{$ah_site_name}/D10-{$ah_site_environment}-{$ah_site_group}-settings.inc",
    "/var/www/site-php/{$ah_site_group}/{$ah_site_group}-settings.inc",
  ];

  foreach ($acquia_settings_candidates as $acquia_settings_file) {
    if (!file_exists($acquia_settings_file)) {
      continue;
    }

    require $acquia_settings_file;
    // Keep config sync in the repo even if Acquia includes define a default.
    $settings['config_sync_directory'] = $app_root . '/../config/sync';
    break;
  }
}
